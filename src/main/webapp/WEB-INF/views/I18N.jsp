<%@ page contentType="text/xml; charset=UTF-8" language="java" errorPage="" import="java.io.*,java.math.*,java.net.*,java.security.*,java.text.*,java.util.*,java.util.zip.*,javax.servlet.http.*,org.w3c.dom.*,websquare.i18n.*,websquare.logging.*,websquare.WebSquareConfig,websquare.util.*,websquare.logging.util.LogUtil"
%><%
...
try {
    ...
    //컨텍스트가 /인 경우 blank.xml만 baseURI 참조를 하고 있지 않아 코드 추가
		String result = null;
		try{
			if ( movePage == null || "".equals(movePage) ) {
				movePage = w2xPath;
			}
			
			if ( !movePage.equals(w2xPath) ) {
				result = "<?xml version=\"1.0\" encoding=\"UTF-8\"?><html xmlns=\"http://www.w3.org/1999/xhtml\" xmlns:ev=\"http://www.w3.org/2001/xml-events\" xmlns:w2=\"http://www.inswave.com/websquare\" xmlns:xf=\"http://www.w3.org/2002/xforms\"><head><w2:buildDate/><xf:model><xf:instance><data xmlns=\"\"/></xf:instance></xf:model><script type=\"javascript\"><![CDATA[ wc.getFrame( wc.getSelectedTabIndex()).setSrc(\""+ movePage +"\"); ]]></script></head><body></body></html>";
			} else {
